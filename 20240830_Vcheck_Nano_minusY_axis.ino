 ...
 *
 *     ***** See Peter Ashwood Smith's GitHub site: https://github.com/peterashwoodsmith ********
 *    
 * vcheck: Engine flywheel/propellor balancer. 
 * ==========================================
 * V1.1 (C) April 2018 Peter Ashwood-Smith (
 * 
 * This little Arduino program will measure the angle and amplitude of imbalance in an engine's flywheel and / or propellor.
 * It does this by tracking the position of the flywheel relative timing mark while simultaneously reading acceleration data 
 ...
 * 360/540 or other 4 stroke engine. It will display the results of the analysis on a small LCD screen. An op-amp is configured as a 
 * voltage follower and is placed near the ADXL335 to buffer the high ADXL335 impedance of 32KOhms. The opamp allows driving the 
 * analog -X acceleration signal via a cable, offering the possibility of a termination (100 Ohms) at the Arduino analog input.
 *
 * An optical sensor (Banner SM312LVMHS) is used to track flywheel position via a reflective (retroflective) tape attached to the flywheel. The active low 
 * signal is fed via a cable and to the Arduino, terminated with a 100 Ohm resistor. In future, the plan is to evaluate other sensors
 * as a lower cost replacement, since the Banner optical sensor price (with 10 meter cable) is approximately $200 US.
 *
 * An isr() on the interrupt pin times the zero degree reference mark on the flywheel and computes RPM and allows computation of the position 
 * of the flywheel between recurring reference marks. It also has some debouncing and sanity checking because occasional spurious interrupts 
 * seem to still plague the design.
 * 
 * A polling loop will continuously read an accelerometer's analog input, each time it samples it will compute the 
 * position of the flywheel in degrees 0..359 relative to the last Optical trigger and populate acceleration average data in an array for that
 * angle. Digital accelerometers of sufficient bandwidth via I2C could also be used as can much higher bandwidth/resolution 
 * analog accelerometers. Normally vibration measurements are 0-2G with 1G being quite rough. So 16g accelerometers and 3 axis 
 ...
 * is performed. The analysis consists of a Discrete Fourier Transform to extract the power and phase of each spectral component of
 * the vibration. We extract the frequency, phase and power of the most significant harmonic. This is the major vibration source.
 * If that frequency is the same as the rotational frequency of the flywheel then the flywheel/propeller is the source of the major
 * Y axis vibration and we will display it on the LCD. We also keep track of the largest of these that we have seen since start up and 
 * display the largest at all times on the LCD.
 */

#include <math.h>           // We use sin() cos() and PI
#include <LiquidCrystal.h>  // Great little LCD dirver object.

const int DEBUG = 0;  // Set to non zero and serial debugging occurs   *****************FOR DEBUG SET TO NON ZERO *****************

const byte interrupt_pin = 2;               // pin that is receiving FALLING EDGE optical trigger interrupts    ************************ digital pin 2 for Nano ************************
const byte accelerationPin = A0;            // pin that is receiving analog X-axis acceleration levels
...
const int NUM_BIN = 180;                    // How many acceleration samples, 2 degree resolution   ********** 3 degree resolution? ***************
long int acc[NUM_BIN];                      // total acceleration measures at angle i
int hit[NUM_BIN];                           // how many times we measured at this angle i
volatile int interrupt_count = 0;           // how many times we've seen a Optical trigger interrupt so far
volatile int valid_interrupt_count = 0;     // how many Optical trigger interrupts seemed sensible
volatile long last_interrupt_time = 0;      // time of last sensible Optical trigger interrupt
volatile int rpm = 0;                       // current RPM as computed by isr()
volatile long dt = 0;                       // delta time between Optical triggers
volatile long rpm_total = 0;                // We try to average the RPM
volatile int max_rpm = 0;                   // We track max RPM we saw
volatile int min_rpm = 6000;                // We track min RPM we saw   *********************** 6000 ????  arbitrary value to detect if min RPM is being overwritten?  ********************
...
LiquidCrystal lcd(8, 9, 4, 5, 6, 7);  // Creates an LCD object. Parameters: (rs, enable, d4, d5, d6, d7)   ********************** Nano R3 Pins  ***************************

/*
 * This is the interrupt service routine. It gets called every time the Optical trigger fires.
 ...
 * use it while polling to determine for a given acceleration what angle that occurred relative to the Optical trigger.
 */
void isr() {                                    // Optical trigger plug just fired
  long interrupt_time = micros();               // what is current time in microseconds
  interrupt_count += 1;                         // Keep track of stats for debugging     ********** shorthand for:   interrupt_count = interrupt_count + 1 ****************
  if (last_interrupt_time != 0) {               // if not first interrupt
    ...
    if (dt > (long)165 * 100) {                 // if more than 16.5ms and  ********* less than ~ 60Hz  ... 3600 RPM *****
      if (dt < (long)60 * 1000) {               // less than 60ms its a valid DT.   **** more than ~ 16Hz.... 960 RPM *************
...
/*
 * This is the workhorse polling loop. It will read the accelerometer input for duration milliseconds repeatedly.
 * The trick here is that the isr() is humming away in the background resetting the last_interrupt_time every time
 * a Optical trigger occurs. We use the time of the last_interrupt and the time 'dt' between interrupts to compute how far the
 * fly wheel will have rotated since the Optical trigger. We then convert that to a number 0..NUM_BIN (NUM_BIN often 360) and add
 * the acceleration we are reading into an average we are keeping at that slot/angle. After a many seconds of polling we
 * will have an array of acc values 0..359 and hit values 0..359. From these we can produce average acceleration at
 * each angle 0..359 for subsequence spectral analysis. To increase the dynamic range of the A2D conversion we use a 
 * 2.56 volt internal analog reference. We really only need to read 1G deviations so the range should be as tight as
 ...
 * errors in the accelerometer (over/under G readings).
 */
int poll(unsigned int duration) {
  ...
  if (end_t < now) {
    delay(duration);
    return;
  }                                  // millis() will wrap just skip this sample.
  /* MICROS WRAP CHECK GOES HERE */  // micros() will wrap just skip this sample.
  while (millis() < end_t) {
    if ((rpm > 0) && (last_interrupt_time > 0) && (dt > NUM_BIN)) {  // if the isr() is giving us RPM and valid last Optical trigger
      int accel = analogRead(accelerationPin);                       // Lets get the accel value and then find its slot
      ...
      if ((accel > 0) && (accel < 1023)) {
        if ((rot >= 0) && (rot < NUM_BIN)) {  // sanity checks
          ...
          hit[rot] += 1;                      // into running average at this rotation angle (0..359).
        }
      } else
...
/*  Now we do a DFT on the acceleration data for a        lot. Usually we will only do this for
 ...
 */
void discreteFourierTransform(int k, float *this_a, float *this_ang) {
  float sum_r = 0.0;
  float sum_i = 0.0;                   // initialize running real/imaginary tallies
  for (int t = 0; t < NUM_BIN; t++) {  // standard slow Fourier iteration for harmonic k
    ...
    float in = (float)acc[t] / (float)hit_t;       // use average of acceleration at this slot
...
void updateLcdLine(int steady, int rpm, int angle, float vel) {
  char temp[10];
  sprintf(temp, "%04d", rpm);
  lcd.print(temp);
  lcd.write(C_RPM);
  lcd.print(" ");  // eg: "2500r/m "
  if (steady) {
...
void updateLcd(int steady, int rpm, int angle, float vel) {
  ...
  if (rpm < 500) {              // below 1700 RPM no estimate  *********** changed to 500 rpm ****************
    if (rpm > 0) {
  ...
  if (best_hits == 0) {             // if we have no estimate then
    lcd.print("NO BEST ESTIMATE");  // just say we have no best estimate
                                    ...
                                    // OLD ************** updateLcdLine(1, best_rpm, best_angle, best_vel);  // Otherwise update best estimate so far.
     lcd.print("-----------------");  // *********** replaced updateLcdLine (above) with Sending a blank (hyphened) 2nd line instead of "BEST" parameters ************
  }
}
 ...
 *  impact on the acceleration measurements because they artifically introduce high frequency 
 ...
 * known 0 angle imbalance and then measure the error at different RPMs.
 */
int correctAngleForRpm(int angle, int rpm) {  //
  ...
  //
  static int corr[][2] = {
    { 1700, -0},   //************ was { 1700,  5 }, **************************************** this corr number is subtracted from display reading
    { 2000, 0 },  //************ was  { 3600,  70 } };  ***********************************
    { 2200, 0 },
    { 2400, 0 },
    { 2700, 0 },
  ...
  // Then do a linear extrapolation to find a reasonable correction for this input angle.
  //
  static int maxi = sizeof(corr) / sizeof(corr[0]);
  for (int i = 0; i < maxi - 1; i++) {
    if ((rpm >= corr[i][0]) && (rpm < corr[i + 1][0])) {  // Find bounding correction pair
      int dx = corr[i + 1][0] - corr[i][0];               // Compute slope between this pair
 ...
 * counters. Then we enable the interrupt on the Optical trigger  circuit to trigger the isr() on a falling edge.
 * After that we just enter the polling loop for some small number of seconds to accumulate all our acceleration
 * data. As mentioned above the polling loop can know at what angle a given acceleration occurred because it
 * has access to the time, the expected time of the next Optical trigger and the previous Optical trigger and can produce the acc and
 * hit arrays as a result. Usually this will be an array of 360 acceleration values. After we have accumulated
 * many rotations of acceleration data we can then run a spectral analysis of the signal. Essentially we run a
 * Discrete Fourier Transform (DFT) but are not interested in all the frequency components, we only want the one
 * with the maximum energy. When the flywheel/prop is out of balance the lowest frequency component will be the 
 * highest energy. When the flywheel/prop is in good balance, there will be some other frequency that is 
 * contributing more energy to the vibration than the flywheel/prop rotation frequency.
 */
void loop() {
  ...
  attachInterrupt(digitalPinToInterrupt(interrupt_pin), isr, FALLING);  // setup interrupts for Optical trigger isr      ************* was attachInterrupt(digitalPinToInterrupt(interrupt_pin), isr, RISING); ***********
  overG = poll(1000);                                                   // continuous poll of accel for 4000ms   **********1000, was 4000, was 2000 **********
  ...
  if (overG > 0) {                                                      // If during polling the acceleratomer went full scale
    ...
    discreteFourierTransform(NUM_BIN - 1, &accelN, &angle);  // find acceleration and phase for N-1st harmonic (this is at RPM frequency).
    discreteFourierTransform(1, &accelone, &angle);          // find acceleration and phase for 1st harmonic (this is at RPM frequency).
    accel = accelN + accelone;                               // RPM harmonic accel in bits is sum of 1st and N-1st spectra in bits
    accelv = (accel * MAXANALOG) / MAXDIGITAL;               // digital acceleration convert to analog relative to reference voltage
    gs = accelv / VOLTSPERG;                                 // amplitude in G is (acceleration volts / 0.330 mv/G)
    ...
    steady = (max_rpm - min_rpm) < 100;                      // If RPM is not steady we don't count a maximum ****Changed temporarily to 200***** from 100
    if (angle > 0)                                           // We convert from FFT phase angle to peak position
      angle = 360 - angle;                                   // positive angles are 1..180 before Optical trigger
    else if (angle < 0)                                      // -1..-180 mean peak is 1..180 after Optical trigger
      angle = -angle;
  ...
  /*  Debugging 
      ...
      *  Finally print a curve of the sampled acceleration data as a graph, one sample per line.    
      */
  if (DEBUG) {
    int i, j;
    int max_acc = 0;
    ...
    Serial.print("          Valid= ");
    Serial.println(valid_interrupt_count);
    Serial.print("           Zero= ");
    Serial.println(zero_time_interrupts);
    Serial.print("          Short= ");
    Serial.println(invalid_interrupts_short);
    Serial.print("           ShDT= ");
    Serial.println(short_dt);
    Serial.print("           Long= ");
    Serial.println(invalid_interrupts_long);
    Serial.println("----------------------------------------------------------");
    Serial.print("      Last RPM = ");
    Serial.println(rpm);
    Serial.print("           MIN = ");
    ...
    Serial.print("       AVG FRQ = ");
    Serial.println(freq);
    Serial.print("     AVG 1/FRQ = ");
    Serial.println((float)1.0 / freq);
    Serial.print("            NZ = ");
    Serial.println(nz);
    Serial.println("----------------------------------------------------------");
    Serial.print(" Vibration IPS = ");
    Serial.println(vel);
    Serial.print("     bits acc1 = ");
    Serial.println(accelone);
    Serial.print("     bits accN = ");
    Serial.println(accelN);
    Serial.print("     volts acc = ");
    Serial.println(accelv);
    Serial.print("           Gs  = ");
    Serial.println(gs);
    Serial.print("       RAW ANG = ");
    Serial.println(angle);
    Serial.print("       COR ANG = ");
    Serial.println(corrected_angle);
    Serial.println("----------------------------------------------------------");


    for (i = 0; i < NUM_BIN; i++) {  // Figure out the maximum acceleration sample
      if (hit[i] > 0) {
    ...
    min_acc = max_acc;
    for (i = 0; i < NUM_BIN; i++) {  // Figure out the minimum non zero acceleration sample
      if (hit[i] > 0) {
        int temp = (acc[i] / hit[i]);
    ...
    if (max_acc > 0) {                 // Now print a little graph, each line is one sample
      ...
      for (i = 0; i < NUM_BIN; i++) {  // This should look like the oscilliscope acceleration data
        if (hit[i] > 0) {
          ...
          if ((acci == max_acc) || (acci == min_acc)) {
            ...
            Serial.print(acci);
            Serial.print("@");
            Serial.print(angle);  // but may not be same as Fourier
          }
        }
        Serial.println("");
      }
...
int selftest() {
  ...
  for (samples = 0; samples < 100; samples++) {  // For 1 second, sample analog every 10ms (i.e. 100 times).
    int v = analogRead(accelerationPin);
    averagebits += v;
  ...
  if (DEBUG) {
    ...
    Serial.print("      volts per G = ");
    Serial.println(VOLTSPERG);
    Serial.print("       max analog = ");
    Serial.println(MAXANALOG);
    Serial.print("      max digital = ");
    Serial.println(MAXDIGITAL);
    Serial.print("       zeroGvolts = ");
    Serial.println(ZEROGVOLTS);
    Serial.print("     minus1Gvolts = ");
    Serial.println(minus1Gvolts);
    Serial.print("      plus1Gvolts = ");
    Serial.println(plus1Gvolts);
    Serial.print("  avg accel volts = ");
    Serial.println(averageAccelVolts);
    Serial.print("  avg accel bits  = ");
    Serial.println(averagebits);
  }
  if (abs(averageAccelVolts - plus1Gvolts) / plus1Gvolts < 0.10) return (0);    // All ok                          // **********BYPASS SENSOR TESTING************* was if (abs(averageAccelVolts - plus1Gvolts)/plus1Gvolts < 0.10)   return(0);
...
void setup() {
  int rc;
  // analogReference(INTERNAL2V56);                                           // analogRead dynamic range is 0..2.56 volts 0..1023 ******************* as measured on AREF for Nano *********************************
  MAXANALOG = 5.00;                      // maximum analog voltage we can measure (will be 1023)    ****************** 5.00V for Nano **************************
  MAXDIGITAL = 1023.0;                   // Dynamic range is 0..1023
  VOLTSPERG = 0.330;                    // Calibrated volts/G shows  0.2375.   ************* changed to 0.3300 for ADXL335 *******************************
  ZEROGVOLTS = 1.640;                   // Calibrated 0G volts shows 1.2275   *********** changed to 1.610  for ADXL335 ********************************
  pinMode(interrupt_pin, INPUT_PULLUP);  // Optical trigger drive interrupts, acceleration is at A0.
  ...
  if (DEBUG) {
    ...
    lcd.print("V-CHECK VER 1.01");  // Otherwise normal banner  *************** was VER 1.00 ******************************************
    lcd.setCursor(0, 1);
  ...
  delay(2000);
  rc = selftest();                                 // Do a quick self test of accelerometer sanity
  if (rc != 0) {                                   // If all ok, let the loop() run we just return
    ...
    while (1) {                                      // Blink the LCD forever
      int accd = analogRead(accelerationPin);        // And for debugging show digital version of analog
      float accv = (accd * MAXANALOG) / MAXDIGITAL;  // what that analog is converted.
      float accg = (ZEROGVOLTS - accv ) / VOLTSPERG;                 // and what that converted to G is.  ******* changed to:  (ZEROGVOLTS - accv ) / VOLTSPERG; not giving correct G, changed formula from accg = accv * VOLTSPERG
      lcd.setCursor(0, 0);                           // *********** ... used negative G reading from sensor to allow more headroom as limit is 2.56V (NOTE: applies to Mega 2560 ONLY) with zero G of 1.83V
      lcd.print(msg);                                // therefore mounted sensor upside down
      lcd.setCursor(0, 1);
      lcd.print(accd);
      lcd.print("/");
      lcd.print(accv);
